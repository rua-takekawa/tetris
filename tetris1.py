...
def main():
    ...
    while True:
        ...
        if game_over:  # ゲームオーバー画面
            ...
            game_over_text = font_large.render("Game Over", True, dark_gray)
            screen.blit(game_over_text, (SCREEN_WIDTH // 2 - game_over_text.get_width() // 2, SCREEN_HEIGHT // 2 + 50))

            game_over_x = SCREEN_WIDTH - game_over_text.get_width() -   150

            # コンテニューボタン
            continue_text = font.render("Press SPACE to Continue", True, dark_gray)
            screen.blit(continue_text, (SCREEN_WIDTH // 2 - continue_text.get_width() // 2, SCREEN_HEIGHT // 2 + 120))
            continue_x = SCREEN_WIDTH // 2 - continue_text.get_width() // 2  # 水平中央
            continue_y = SCREEN_HEIGHT - 50  # 画面下部から100px上に配置
